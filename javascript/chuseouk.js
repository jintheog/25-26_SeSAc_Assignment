...
/**
 * 요소 element : 웹페이지의 구조와 컨텐츠를 만드는 기본 단위. 태그, 속성, 컨텐츠로 이루어져 있다.
 * 태그 : 요소의 기능과 역할을 결정. 시작태그와 종료태그가 있지만 종료태그가 없는 태그도 존재.
 * 속성 attribute : 요소에 추가적인 정보와 기능을 제공. 속성이름(속성명) = "값" 형태.
 *    e.g. href: a 태그 고유 속성. 클리깃 이동할 주소를 지정, id: 고유 식별자 부여, class: 요소들을 그룹화, style: 인라인 스타일 지정
 *          src : 이미지, 비디오 파일 등 파일의 경로를 지정
 * 컨텐츠 : <body></body>태그에 들어가는 요소가 감싸고 있는 실제 내용. 테스트, 이미지, 다른 요소, 태그, 링크 등.
 */

// 문제 [개념]
// HTML 속성 중 class 속성의 역할에 대해 작성한다
/**
 * class: 요소들을 그룹화 하는 속성. 하나의 요소에 여러 class를 지정 하거나 하나의 class를 여러 요소에 지정 할 수 있다.
 */
// 문제 [개념]
...
/**
 * let : 변수를 선언 할때 쓰는 문법. 같은 변수를 다른 값으로 재할당이 가능 하고 호이스팅이 되지만 초기화 전에는 접근 불가. 선언과 동시에 초기화 하지 않아도 된다.
 * const : 변수를 상수로 선언할 때 쓰는 문법. 재할당이 불가능 하고 호이스팅이 되지만 초기화 전에는 접근 불가. 선언과 동시에 초기화 해야 한다.
 */
// 문제 [실습]
 ...
 * a >= b: a가 b보다 같거나 클 경우
 * b >= a:  b가 a보다 같거나 클 경우
 * a === b: a와 b가 엄격하게 같을 경우. 타입과 값이 둘 다 같아야 true
 * a !== b: a와 b가 엄격하게 다를 경우. 타입과 값이 둘 다 달라야 false
 */
// 문제 [실습]
...
// 그리고 상수 status를 출력한다
let age = 17;
const userStatus = age > 18 ? "성인" : "미성년자";
...
/**
 * 배열 생성 방법 : 배열이름 = []; 대괄호를 사용해 배열 생성. e.g. let arr = [];
 * 배열 요소 접근 방법 : [] 대괄호와 인덱스로 배열 원소에 접근. e.g. arr[0], arr[1], arr[2]
 * 배열 원소 수정 방법 : [] 대괄호와 인덱스로 접근 하여 원소에 데이터 재할당. e.g. arr[0] = 10;
 * 배열 요소 추가 방법 : - push()로 배열 끝에 원소 추가. e.g. arr.push(3);
 *  - unshift()로 배열 맨 앞에 추가. e.g. arr.unshift(1);
 *  - 인덱스 직접 지정하여 추가. e.g. arr[3] = 50;
 * 배열 요소 삭제 방법 : pop()로 배열의 마지막 원소를 제거 하고 반환. e.g. let popNum = arr.pop();
 *  - shift()로 맨 앞 원소 제거. e.g. arr.shift();
 */
...
/**
 * 객체 생성 방법 : {} 중괄호를 사용해 생성. e.g. let object = {key: value, key2: value2,};
 * 객체 속성 접근 방법 : . 마침표 혹은 [] 대괄호로 접근 (key에 변수, 공백, 특수문자가 사용된 경우는 [] 중광호를 이용해서 접근) e.g. person.name, person.["job"]
 * 객체 속성 추가 방법 : . 또는 []로 속성 추가. e.g. person.address  = "서울"; person["country"] = "한국"
 * 객체 속성 삭제 방법 : delete 연산자를 사용 하여 삭제. delete person.age;
 */
// 문제 [실습]
...
const array1_2 = [...array1];

// 아래 주석을 풀어서 두 변수(array1, array2)의 비교 결과를 출력한다
console.log(array1 === array1_2); // false
